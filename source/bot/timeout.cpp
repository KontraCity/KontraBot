...
void Bot::Timeout::threadFunction()
{
    ...
    bool timeouted = !m_cv.wait_for(lock, std::chrono::minutes(m_timeoutDuration), [this]() { return !m_enabled; });
    m_enabled = false;
...
Bot::Timeout::Timeout(const Callback& callback, unsigned int timeoutDuration)
    : m_enabled(true)
    , m_callback(callback)
